  ...
  export default {
    ...
    methods: {
      ...
      async initData () {
        ...
        // console.log(this.lendData.data.items.sort(compare))
        ...
        for (let i = 0; i < 366; i++) {
          ...
          while (index < sorted.length && sorted[index].settledAt <= future) {
            if (sorted[index].settledAt >= now) {
              currentBalance += (parseFloat(sorted[index].interest) - (parseFloat(sorted[index].interest) * 0.15))
        ...
        groupData.forEach(array => {
          ...
          if (array.array.length !== 0) {
            ...
            // })
            // rate = rate / array.array.length
            graphData.push({
              x: new Date(array.date),
              y: (array.balance).round(3)
            })
          }
        })
